...
const TransporteColetivoSellingPage = () => {
  ...
  useEffect(() => {
    if (location && location.pathname.includes('formacao')) {
      ...
      setCourseRequirements(['Ter o curso de capacitação de Transporte Coletivo de Pssageiros descrito no campo de observações da CNH']);
  ...
  useEffect(() => {
    if (availableCourses.length > 0) {
      setCourseInfo(availableCourses.filter((course) => (course[0].slice(0, 8) === process.env.REACT_APP_TRANSPORTE_COLETIVO_BASE_ID) && (location.pathname.includes(course[1].type))));
    }
  }, [availableCourses]);
...
export default TransporteColetivoSellingPage;